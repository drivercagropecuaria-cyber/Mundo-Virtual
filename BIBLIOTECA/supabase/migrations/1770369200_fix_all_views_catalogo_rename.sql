-- Migration: fix_all_views_catalogo_rename
-- Created at: 1770369200  
-- Purpose: Rebuild all views to use new 'catalogo' table name after migration 1770369100 renamed catalogo_itens -> catalogo
-- Note: This migration runs AFTER the rename and fixes all dependent views

-- Drop all dependent views in correct order (reverse dependency)
...
-- v_catalogo_ativo: Simple view of non-deleted catalog items
CREATE OR REPLACE VIEW v_catalogo_ativo AS
SELECT * FROM catalogo
WHERE deleted_at IS NULL;

-- v_catalogo_completo: Complete view with all taxonomies and media
...
-- v_catalogo_stats: Statistics view
...
-- v_catalogo_audit_recente: Recent audit log view
...
VALUES ('catalogo', 'SCHEMA_CHANGE', NULL, NULL, 'Rebuilt all views to use new catalogo table name', NOW())
